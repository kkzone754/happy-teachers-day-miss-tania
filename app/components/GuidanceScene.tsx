...
function MemoryIcon({ type }: { type: string }) {
  if (type === "book") {
    return (
      <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7 sm:h-8 sm:w-8">
        <path d="M4 5.5C6.4 4.7 8.8 5 11 6.3V19c-2.2-1.3-4.6-1.6-7-.8V5.5Z" stroke="currentColor" strokeWidth="1.4" />
        <path d="M20 5.5c-2.4-.8-4.8-.5-7 0.8V19c2.2-1.3 4.6-1.6 7-.8V5.5Z" stroke="currentColor" strokeWidth="1.4" />
        <path d="M12 6.5V19" stroke="currentColor" strokeWidth="1.4" />
      </svg>
    );
  }
  ...
  if (type === "mic") {
    return (
      <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7 sm:h-8 sm:w-8">
        ...
        <path d="M6.5 11.5a5.5 5.5 0 0 0 11 0M12 17v4M8.5 21h7" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  if (type === "mask") {
    return (
      <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7 sm:h-8 sm:w-8">
        <path d="M4 7.5c2.7-1.8 5.4-2 8-1.1 2.6-.9 5.3-.7 8 1.1v5.2c0 4.1-3.3 6.8-8 7.8-4.7-1-8-3.7-8-7.8V7.5Z" stroke="currentColor" strokeWidth="1.3" />
        <path d="M7.5 11.5c.8-.7 1.7-.7 2.5 0M14 11.5c.8-.7 1.7-.7 2.5 0M9 15.5c2 1.1 4 1.1 6 0" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
      </svg>
    );
  }
  ...
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7 sm:h-8 sm:w-8">
      <path d="m12 3 2.7 5.5 6.1.9-4.4 4.3 1 6.1-5.4-2.9-5.4 2.9 1-6.1-4.4-4.3 6.1-.9L12 3Z" stroke="currentColor" strokeWidth="1.3" strokeLinejoin="round" />
    </svg>
  );
}
...
export default function GuidanceScene() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const progressDots = q(".guidance-progress-dot");

      gsap.set(chapter, { opacity: 0, y: 15 });
      ...
      gsap.set(memoriesEl, { opacity: 0, x: 50, y: 15 });
      gsap.set(final, { opacity: 0, y: 50, scale: 0.96 });
      gsap.set(spine, { scaleY: 0, transformOrigin: "top center" });
      gsap.set(progressDots, { opacity: 0.25, scale: 0.7 });

      const intro = gsap.timeline();
      intro
        .to(chapter, { opacity: 1, y: 0, duration: 0.6, ease: "power3.out" })
        .to(eyebrow, { opacity: 1, y: 0, duration: 0.65, ease: "power3.out" }, "-=0.3")
        .to(title, { opacity: 1, y: 0, duration: 0.95, ease: "power4.out" }, "-=0.35")
        .to(introLine, { scaleX: 1, duration: 0.65, ease: "power3.out" }, "-=0.45")
        .to(ghost, { opacity: 0.055, scale: 1, duration: 1.2, ease: "power3.out" }, "-=0.7");
      ...
      timeline
        ...
        .to(eyebrow, { opacity: 0, y: -20, duration: 0.45 }, "<")
        .to(ghost, { opacity: 0.025, scale: 1.04, duration: 0.7 }, "<")
        .to(introLine, { opacity: 0, duration: 0.3 }, "<");
      ...
      timeline
        .to(spine, { scaleY: 1, duration: 1.2, ease: "power2.out" }, "<")
        .to(
          memoriesEl,
          {
            opacity: 0,
  ...
  return (
    <section
      ref={sectionRef}
      className="relative h-screen overflow-hidden bg-[#070504] px-6 text-[#fff8e8]"
    >
      <div className="pointer-events-none absolute inset-0">
        ...
        <div className="absolute left-0 top-1/2 h-px w-full bg-gradient-to-r from-transparent via-[#d9a64c]/[0.08] to-transparent" />
      </div>
      ...
      <div className="guidance-chapter absolute left-6 top-7 z-30 flex items-center gap-3 sm:left-10 sm:top-10">
        ...
        <span className="font-sans text-[10px] tracking-[0.25em] text-white/45">
          03
        </span>
      </div>

      <div className="relative z-10 mx-auto flex h-full w-full max-w-[1500px] items-center">
        <div className="relative grid w-full grid-cols-1 gap-10 lg:grid-cols-[minmax(0,0.92fr)_minmax(480px,0.82fr)] lg:gap-16">
          <div className="relative flex min-h-[500px] items-center lg:min-h-0">
            <div
              className="guidance-ghost pointer-events-none absolute left-[4%] top-1/2 -translate-y-1/2 whitespace-nowrap font-display text-[24vw] font-semibold uppercase leading-none tracking-[-0.1em] text-[#f0d49a] sm:text-[18vw] lg:left-[-5%] lg:text-[15vw]"
            >
              CONFIDENCE
            </div>
          ...
          <div className="relative min-h-[430px] lg:min-h-[500px]">
            ...
            <div className="absolute right-0 top-1/2 hidden -translate-y-1/2 flex-col items-center gap-5 sm:flex">
              {memories.map((memory) => (
                <span
                  key={memory.number}
                  className="guidance-progress-dot h-1.5 w-1.5 rounded-full border border-[#d9a64c]/80 bg-[#070504]"
                />
              ))}
            </div>
            ...
            <div className="relative h-full pl-9 sm:pl-14">
              {memories.map((memory) => (
                ...
                >
                  <div className="group relative overflow-hidden rounded-[2px] border border-[#d9a64c]/[0.14] bg-black/20 px-5 py-5 backdrop-blur-sm transition-colors duration-500 sm:px-7 sm:py-6">
                    <div className="absolute inset-0 bg-gradient-to-r from-[#d9a64c]/[0.045] via-transparent to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                    <div className="absolute left-0 top-0 h-full w-px bg-gradient-to-b from-transparent via-[#d9a64c]/70 to-transparent" />

                    <div className="relative flex items-start gap-4 sm:gap-5">
                      <div className="flex shrink-0 flex-col items-center">
                        <div className="flex h-14 w-14 items-center justify-center rounded-full border border-[#d9a64c]/40 bg-[#0b0805] text-[#e4b96a] shadow-[0_0_28px_rgba(217,166,76,0.08)] sm:h-[68px] sm:w-[68px]">
                      ...
                      <div className="min-w-0 flex-1 pt-1">
                        <div className="flex items-center gap-3">
                          <span className="font-display text-2xl font-medium tracking-[-0.03em] text-[#f0d49a] sm:text-3xl">
                            {memory.number}
                          </span>
